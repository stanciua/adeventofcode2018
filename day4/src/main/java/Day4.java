...
//            System.out.print(sum + " " + Arrays.stream(getSleepTimeById(record.id)).mapToObj(i -> String.valueOf(i)).reduce("", (acc, x) -> acc + x + " "));
        ...
        System.out.println(mostSleepingId);
...
class Record {
    ...
    Record(String record) {
        ...
        if (match.matches()) {
            time = LocalDateTime.of(Integer.valueOf(match.group(1)), Integer.valueOf(match.group(2)), Integer.valueOf(match.group(3)), Integer.valueOf(match.group(4)), Integer.valueOf(match.group(5)));
            id = Integer.valueOf(match.group(6));
            state = State.BEGINS_SHIFT;
        }
