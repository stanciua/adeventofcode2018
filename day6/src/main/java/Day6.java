...
class Day6 {
    ...
    private int calculateAreaSize(String[][] gridChars, String ch) {
        return (int)Arrays.stream(gridChars).flatMap(Arrays::stream).filter(c -> c.toLowerCase().equals(ch.toLowerCase())).count();
    }
    ...
    private int calculateSizeForCoordinate(String[][] gridChars, List<int[]> coordinates, int cIndex) {
        String ch = "C" + cIndex;
        int x = coordinates.get(cIndex)[0];
        int y = coordinates.get(cIndex)[1];
        // check to see if this coordinate is infinite:
        //   - if any of the extremities have the value of the coordinate we are searching for then this should be
        //     flagged as infinite area
